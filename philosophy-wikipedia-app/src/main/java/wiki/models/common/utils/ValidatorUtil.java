package wiki.models.common.utils;
...
import wiki.models.common.lang.ValidationException;
	...
	public static <T> void validateEntityFields(final Validator validator, final T entity) {
		...
		if (!violations.isEmpty()) {
			final Set<String> violationMessages = violations.stream()
					.map(v -> String.format("%s %s", v.getPropertyPath(), v.getMessage())).collect(Collectors.toSet());
